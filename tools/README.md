...
`align` 算完共同畫布之後，把整套等比縮到不超過這個高度。給 `0` 或不給就不限制。

它存在的理由是出貨體積：mycat 原生尺寸整套 39MB，`--max-height 384` 之後 7.1MB，
而 384 高在 Retina 上仍然足夠。`build-mycat.py` 寫死用它。

縮圖走預乘 alpha（先乘上 alpha、LANCZOS、再還原），不是直接對 unpremultiplied
RGBA 做——後者會讓透明像素的顏色滲進邊緣，長出一圈色暈。anchor 是相對座標，
等比縮放不改變它。

負值直接硬失敗，而且**在碰檔案系統之前就擋**：排在後面的話，打錯旗標配上一個
沒有動作子目錄的路徑會回 `NO_ACTIONS`，把人指去查目錄結構。
