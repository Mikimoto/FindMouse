...
public struct PackVersion: Sendable, Equatable, Comparable {
    ...
    ///
    /// 只認 ASCII 數字。**擋掉的是 `Int` 認得而版本字串不該認的東西**：
    /// `Int("+1")` 回 1、`Int("-1")` 回 -1（2026-08-13 實測），於是 `+1` 會被
    /// 當成版本 1、`1.-2` 會被當成 `1.-2` 的負數段。全角數字則是 `Int` 自己就
    /// 擋掉的（同日實測 `Int("１")` 是 **nil**，這裡原本寫「會成功」是錯的）。
    ///
    ...
    public static func parse(_ raw: String?) -> PackVersion? {
        ...
        for p in parts {
            ...
            // 空段落的是 `Int(p)` 而不是它。**兩個判準是 AND，交換順序拒絕的
            // 集合完全相同**——`allSatisfy` 唯一擋得到而 `Int` 擋不到的是帶正負
            // 號的段落（`+1` / `-1`，見 `parse` 的 doc）。
