...
- **`Scripts/test-release.sh` 不會重建陳舊的 `build/FindMouse.app`**——它只在那個目錄
  **不存在**時才建。所以一次中途失敗的 `make-app.sh`（例如停在簽章前的某個守衛）
  會留下一個沒簽章、少檔案的半成品，而下一次 `test-release.sh` 會報一堆與你的 diff
  完全無關的紅。判別訊號：訊息裡出現「對一份真的沙盒 .app 說不」這種自相矛盾的話。
  先 `Scripts/make-app.sh` 重建再重跑。
